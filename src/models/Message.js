...
export class Message extends Model {
    ...
    get previewUrl() {
        ...
        if (this._data.prevUrl) return this._data.prevUrl
        return this._data.previewUrl
    }
    set previewUrl(value) {
        debbuger
        this._data.previewUrl = value
    }
    get info() {
    ...
    get fileName() {
        ...
        if (this._data.filename) return this._data.filename
    ...
    getViewElement(isMe = true) {
        ...
        // console.log('div antes de append statusViewElement ', div)
        switch (this.type) {
            case 'contact':
                div.innerHTML =
                    `
            ...
            case 'image':
                ...
                div.querySelector('.message-photo').on('load', e => {
                    // console.log("div.querySelector('._340lu') >>> ", div.querySelector('._34Olu'))
            ...
            case 'document':
                ...
                <div class="_3_7SH _1ZPgd">
                    <div class="_1fnMt _2CORf">
                        <a class="_1vKRe" href="#">
                            <div class="_2jTyA" style="background-image: url(${this.previewUrl})"></div>
                            <div class="_12xX7">
                                <div class="_3eW69">
        ...
        if (isMe) {
            ...
            // console.log('div depois de append statusViewElement ', div)
        }
        div.firstElementChild.classList.add(className)
        return div
    }

    static upload(fileToSend, from) {

        return Upload.send(fileToSend, from)
    }
    ...
    static sendAudio(chatId, from, file, metadata, photo) {
        return Message.send(chatId, from, 'audio', '').then(msg => {
    ...
    static sendDocument(chatId, from, docToSend, filePreview, info) {

        Message.send(chatId, from, 'document', '').then(msgRef => {

            Message.upload(docToSend, from).then(fileurl => {

                let url = fileurl

                if (filePreview) {

                    Message.upload(filePreview, from).then(prevUrl => {
                        let preUrl = prevUrl
    ...
    static sendImage(chatId, from, imageToSend) {

        return new Promise((s, f) => {

            Message.upload(imageToSend, from).then(fileurl => {

                let downRef = uploadTask.snapshot.ref
                downRef.getDownloadURL().then(url => {
                    console.log('url >>> ', url)
                    return Message.send(chatId, from, 'image', fileurl).then(() => {
                        s() // success
                    })
                })
            ...
            // let uploadTask = Firebase.hd().ref(from).child(Date.now() + '_' + imageToSend.name).put(imageToSend)
            ...
            // downRef.getDownloadURL().then(url => {
            //     console.log('url >>> ', url)
            //     return Message.send(chatId, from, 'image', url).then(()=>{ 
            //         s() // success
            //     }) 
            // })
    ...
    //sends a message to the specified chatId in the argument
    //with the content of the 4th argument
    static send(chatId, from, type, content) {
        console.log('sending message from image input')
        return new Promise((s, f) => {
            return Message.getAllMessagesForActiveContact(chatId).add({
                content,
                timeStamp: new Date(),
                status: 'waiting',
                type,
                ...
                console.log('retorno do envio da mensagem >> result', result)
                ...
                docRef.set({
                    ...
                    console.log('retorno do merge do método envio', message)
                    s(docRef) //
                })
            })
        }).catch(err => {
            console.error('trying to catch exception >> ', err)
        })
    }

    // recover from the chats collection on the specified 'chatId' the conversation 
    static getAllMessagesForActiveContact(chatId) {
        return Firebase.db().collection('chats').doc(chatId).collection('messages')
    }

    // esse método desenha na mensagem enviada o ícone de status da mensagem no canto inferior direito
    getStatusViewElement() {
        // console.log('entrou no elementStatusView')
        // console.log('status da mensagem?? >> ' , this.status)
        let div = document.createElement('div')
        ...
        // console.log('está retornando o statusViewElement div >>> ' , div)
        return div
    }
}